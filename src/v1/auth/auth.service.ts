  ...
  async login(loginDto: LoginDto) {
    ...
    if (loginDto.loginType === LoginType.EMAIL) {
      if (!loginDto.email || !loginDto.password) {
        throw new UnauthorizedException('Email and password are required for email login.');
      }
      user = await this.usersService.findOneByEmail(loginDto.email);
    } else if (loginDto.loginType === LoginType.USERNAME) {
      if (!loginDto.username || !loginDto.password) {
        throw new UnauthorizedException('Username and password are required for username login.');
      }
      user = await this.usersService.findOneByUsername(loginDto.username);
    } else {
      throw new UnauthorizedException(`Login type ${loginDto.loginType} is not supported via password login.`);
    }
